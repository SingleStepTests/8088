...
# Utility to add a 'hash' key to each test in a JSON test suite file.
...
def hash_json_object(obj):
    ...
    return hashlib.sha1(obj_str).hexdigest()
    ...
    for file in files:
        ...
        with open(file_to_process, 'r') as f:
            try:
                ...
                # Add 'hash' key to each test object
                for idx, test_obj in enumerate(test_data):
                    test_obj['hash'] = hash_json_object(test_obj)
